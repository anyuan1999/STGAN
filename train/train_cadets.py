...
def main():
    ...
    # Load data
    with open("../data/cadets/cadets_train.txt") as f:
        data = f.read().split('\n')
    data = [line.split('\t') for line in data if line]
    df = pd.DataFrame(data, columns=['actorID', 'actor_type', 'objectID', 'object', 'action', 'timestamp'])
    df = df.dropna()
    df.sort_values(by='timestamp', ascending=True, inplace=True)
    df = add_attributes(df, "../data/cadets/ta1-cadets-e3-official.json.1")
    ...
    model_path = "word2vec_cadets_model.model"
    ...
    for epoch in range(100):
        ...
        torch.save(gat_model.state_dict(),
                   f'../trained_weights/cadets/gat{epoch}.pth')
        torch.save(tgn_model.state_dict(),
                   f'../trained_weights/cadets/tgn{epoch}.pth')
        torch.save(model.state_dict(),
                   f'../trained_weights/cadets/selfattention{epoch}.pth')
