...
def main():
    ...
    # Load data
    with open("../data/theia/theia_train.txt") as f:
        data = f.read().split('\n')
    data = [line.split('\t') for line in data if line]
    df = pd.DataFrame(data, columns=['actorID', 'actor_type', 'objectID', 'object', 'action', 'timestamp'])
    df = df.dropna()
    df.sort_values(by='timestamp', ascending=True, inplace=True)
    df = add_attributes(df, "../data/theia/ta1-theia-e3-official-1r.json")
    ...
    model_path = "word2vec_theia_model.model"
    ...
    for epoch in range(100):
        ...
        torch.save(gat_model.state_dict(),
                   f'../trained_weights/theia/gat{epoch}.pth')
        torch.save(tgn_model.state_dict(),
                   f'../trained_weights/theia/tgn{epoch}.pth')
        torch.save(model.state_dict(),
                   f'../trained_weights/theia/selfattention{epoch}.pth')
