...
def main():
    ...
    with open("../data/cadets/cadets_test.txt") as f:
        data = f.read().split('\n')
    data = [line.split('\t') for line in data if line]
    df = pd.DataFrame(data, columns=['actorID', 'actor_type', 'objectID', 'object', 'action', 'timestamp'])
    df = df.dropna()
    df.sort_values(by='timestamp', ascending=True, inplace=True)
    df = add_attributes(df, "../data/cadets/ta1-cadets-e3-official-2.json")
    with open("../data/cadets/cadets.json", "r") as json_file:
    ...
    for m_n in range(100):
        ...
        gat_model.load_state_dict(
            torch.load(
                f'../trained_weights/cadets/gat{m_n}.pth',
                map_location=device))
        gat_model.eval()
        tgn_model.load_state_dict(
            torch.load(
                f'../trained_weights/cadets/tgn{m_n}.pth',
                map_location=device))
        tgn_model.eval()
        model.load_state_dict(
            torch.load(
                f'../trained_weights/cadets/selfattention{m_n}.pth',
                map_location=device))
        model.eval()
