...
def main():
    ...
    with open("../data/theia/theia_test.txt") as f:
        data = f.read().split('\n')
    ...
    for m_n in range(100):
        ...
        gat_model.load_state_dict(
            torch.load(
                f'../trained_weights/theia/gat{m_n}.pth',
                map_location=device))
        gat_model.eval()
        tgn_model.load_state_dict(
            torch.load(
                f'../trained_weights/theia/tgn{m_n}.pth',
                map_location=device))
        tgn_model.eval()
        model.load_state_dict(
            torch.load(
                f'../trained_weights/theia/selfattention{m_n}.pth',
                map_location=device))
        model.eval()
